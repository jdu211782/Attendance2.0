...
export const getMonthlyAttendanceData = async (year: number, month: number) => {
    try {
      ...
      console.log('URL запроса (Monthly):', 'https://attendance-backend-24xu.onrender.com/api/v1/user/monthly');
      console.log('Токен авторизации (Monthly):', token ? 'Присутствует' : 'Отсутствует');
  
      const response = await axios.get('https://attendance-backend-24xu.onrender.com/api/v1/user/monthly', {
        params: {
